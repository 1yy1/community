        ...
        Page<DiscussPost> search =elasticsearchTemplate.queryForPage(searchQuery, DiscussPost.class, new SearchResultMapper() {
            ...
            public <T> AggregatedPage<T> mapResults(SearchResponse response, Class<T> aClass, Pageable pageable) {
                ...
                for (SearchHit hit:hits){
                    ...
                    //处理高亮信息
                    HighlightField titleFidld = hit.getHighlightFields().get("title");
                    if (titleFidld!=null){
                        post.setTitle(titleFidld.getFragments()[0].toString());
                    }
                    HighlightField contentFidld = hit.getHighlightFields().get("content");
                    if (contentFidld!=null){
                        post.setContent(contentFidld.getFragments()[0].toString());
                    }
                    list.add(post);
                }
